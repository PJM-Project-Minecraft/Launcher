...
> ⚠️ **Этот мод НЕ собран и НЕ протестирован в игре в этом репозитории** (нет Gradle/
> NeoForge-тулчейна и игрового рантайма). Код ниже — **референс-реализация**: собери его
> `gradlew build` под СВОЮ версию NeoForge, поправь network-API под неё (он меняется между
> 1.21.x), и **обкатай на dev-сервере в репорт-онли**, прежде чем включать enforce на проде.
> Иначе баг в хэндшейке кикнет **всех**. Это тот же поэтапный rollout, что и у прошлых фаз
> античита (`ANTICHEAT_REQUIRE_ATTESTATION` тоже включали после раздачи и обкатки).
