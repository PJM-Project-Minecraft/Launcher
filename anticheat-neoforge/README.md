...
> **два** валидных jar'а (mods.toml + классы на месте, 0 предупреждений). Network-API проверен.
>
> **Два раздельных артефакта (общий modId `pjmac` → канал совпадает):**
> - `build/libs/anticheat-neoforge-0.1.0-client.jar` — **в профиль игроков** (`mods/`).
>   Только ответ на challenge (HMAC). Ни секрета, ни URL-ов, ни серверной логики.
> - `build/libs/anticheat-neoforge-0.1.0-server.jar` — **на игровой сервер** (`mods/`).
>   Выдача challenge, верификация через бэкенд, опрос отзывов доступа.
> - `anticheat-neoforge-0.1.0.jar` (без classifier) — **НЕ раздавать**: содержит обе
>   `@Mod("pjmac")`-точки входа, в рантайме упадёт на дубле modId. Это build-полуфабрикат.
>
> ✅ **Обкатан на dev-сервере в репорт-онли (2026-07-26): грузится и работает** — хэндшейк и
> опрос отзывов проходят. `enforce` в игре ещё НЕ проверялся (прод в репорт-онли).
>
> ⚠️ **БЛОКЕР перед продом: канал обязательный** (`event.registrar("1")` без `.optional()`) —
> игрок БЕЗ клиентского мода отлетит на входе на сетевом уровне, репорт-онли до `/p5/verify`
> даже не дойдёт. Перед раскаткой серверного jar на прод: либо `.optional()` (тогда репорт-онли
> реально безопасен), либо раздать `*-client.jar` ВСЕМ игрокам заранее. Enforce включать так же
> поэтапно, как `ANTICHEAT_REQUIRE_ATTESTATION` — после 100% раздачи и чистых логов.
>
...
## Отзыв доступа в игре (P-1, готово)

Хэндшейк на входе ловит чужой клиент, но не помогает против «прибил агента уже в игре».
Это закрывает **опрос отзывов**: раз в 25с сервер-мод (`P5RevokePoller`) шлёт бэкенду
список онлайн-ников, бэкенд возвращает, кого кикнуть, сервер исполняет.

```
каждые ~25с:
  POST {API}/api/anticheat/p5/revoked   Header: X-AC-P5-Secret
  Body: {"players":["Ник1","Ник2",...]}
  ← {"kick":[{"player":"Ник2","reason":"агент античита перестал отвечать"}]}
     (репорт-онли → пустой kick + reportOnly:true, но бэкенд пишет в лог)
  Сервер-мод: для каждого из kick — disconnect(reason).
```

Что попадает в отзыв (бэкенд, `revoke.go`): **молчание heartbeat агента** (агента убили
в живой игре — раньше был лишь Telegram-алерт) и **detect-kick** (severity-детект гасил
сессию, но игрок оставался на сервере). Отзыв живёт 10 мин и снимается новым запуском
игры (успешный `Confirm`). Fail-open на всех уровнях: бэкенд недоступен / не 2xx / кривой
JSON → сервер не кикает никого. Enforce общий с verify — тот же `ANTICHEAT_P5_ENFORCE`.

## Бэкенд (готово)

- `POST /api/anticheat/p5/verify` — вход-хэндшейк, `backend/internal/anticheat/p5.go`.
- `POST /api/anticheat/p5/revoked` — опрос отзывов, `p5.go` + `revoke.go` (оттестировано).
- Прод `.env`:
...
3. **Dev-сервер:** поставь `*-server.jar` на сервер, `*-client.jar` — в профиль, зайди клиентом со свежим лаунчером (клиентский
