...
mkdir -p "$CACHE_DIR/home/.npm/_cacache" "$CACHE_DIR/cargo/registry" "$CACHE_DIR/cargo/git"
...
docker run --rm \
  ...
  -e PATH=/opt/cargo/bin:/usr/local/bin:/usr/bin:/bin \
  -v "$ROOT_DIR:/work:ro" \
...
SOURCE_BIN="$ROOT_DIR/src-tauri/target/x86_64-unknown-linux-gnu/release/project-minecraft-launcher"
[[ -x "$SOURCE_BIN" ]] || { echo "ERROR: Linux release binary is missing" >&2; exit 1; }
