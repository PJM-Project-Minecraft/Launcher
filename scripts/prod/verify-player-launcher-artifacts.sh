...
Usage: scripts/prod/verify-player-launcher-artifacts.sh \
  ...
  --api-url https://production.example --commit FULL_SHA
EOF
}
...
[[ "$SOURCE_COMMIT" =~ ^[0-9a-f]{40}$ ]] || { echo "ERROR: source commit must be a full SHA-1" >&2; exit 2; }
...
for path in "$linux_bin" "$windows_bin" "$linux_archive" "$windows_archive" \
  "$linux_dir/signature.txt" "$windows_dir/signature.txt" \
  "$linux_dir/BUILD-INFO.txt" "$windows_dir/BUILD-INFO.txt"; do
  [[ -s "$path" ]] || { echo "ERROR: missing artifact: $path" >&2; exit 1; }
done

for info in "$linux_dir/BUILD-INFO.txt" "$windows_dir/BUILD-INFO.txt"; do
  grep -Fqx "source_commit=$SOURCE_COMMIT" "$info" || { echo "ERROR: source commit mismatch in $info" >&2; exit 1; }
  grep -Fqx "api_url=$API_URL" "$info" || { echo "ERROR: API URL provenance mismatch in $info" >&2; exit 1; }
  grep -Fqx "update_pubkey=$UPDATE_PUBKEY" "$info" || { echo "ERROR: update key provenance mismatch in $info" >&2; exit 1; }
  grep -Fqx "manifest_pubkey=$MANIFEST_PUBKEY" "$info" || { echo "ERROR: manifest key provenance mismatch in $info" >&2; exit 1; }
done
...
linux_format="$(file -b "$linux_bin")"
windows_format="$(file -b "$windows_bin")"
grep -Eq '^ELF 64-bit LSB (pie )?executable, x86-64,' <<<"$linux_format" || {
  echo "ERROR: Linux artifact is not an ELF x86-64 executable: $linux_format" >&2
  exit 1
}
grep -Eq '^PE32\+ executable .*x86-64' <<<"$windows_format" || {
  echo "ERROR: Windows artifact is not a PE32+ x86-64 executable: $windows_format" >&2
  exit 1
}
...
for binary in "$linux_bin" "$windows_bin"; do
  ...
  grep -aq "$API_URL" "$binary" || { echo "ERROR: production API URL missing in $binary" >&2; exit 1; }
done
...
cmp -s "$linux_dir/BUILD-INFO.txt" "$verify_dir/project-minecraft-launcher-$VERSION-linux-x64/BUILD-INFO.txt" || {
  echo "ERROR: Linux archive does not contain verified provenance" >&2
  exit 1
}
cmp -s "$windows_dir/BUILD-INFO.txt" "$verify_dir/project-minecraft-launcher-$VERSION-windows-x64/BUILD-INFO.txt" || {
  echo "ERROR: Windows archive does not contain verified provenance" >&2
  exit 1
}
(
  cd "$ARTIFACT_DIR"
  ...
  sha256sum \
    ...
    "project-minecraft-launcher-$VERSION-linux-x64/BUILD-INFO.txt" \
    ...
    "project-minecraft-launcher-$VERSION-windows-x64/BUILD-INFO.txt" \
    >> SHA256SUMS
)
