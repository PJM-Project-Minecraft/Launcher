...
func (h Handler) RegisterRoutesWithV1BridgeUntil(app *fiber.App, authMiddleware fiber.Handler, until time.Time) {
	...
	// Socket намеренно находится вне защищённых prefix /api/admin и
	// /api/profiles: их middleware перехватывают browser upgrade, у которого по
	// стандарту нет Authorization header. Доступ всё равно закрыт одноразовым
	// билетом, выданным защищённым admin endpoint выше.
	app.Get("/api/realtime/profiles/ws", h.requireSocketTicket, websocket.New(h.eventsSocket, websocket.Config{
