...
- `src-tauri/` — Tauri 2 / Rust-ядро десктоп-лаунчера. Текущая версия: `0.5.4` (в `Cargo.toml`).
...
### Зеркало бэкенда (выбор сервера)

Лаунчер умеет ходить на бэкенд через зеркало: селектор «Сервер» есть на окне входа
и в настройках (после логина окно входа уже не показать, а домен может начать резаться
в любой момент). Оба селектора — один список и один обработчик; скрыты, пока зеркало
одно (`server-visible`).

**Нулевой пункт списка — «Авто» (дефолт).** При старте лаунчер пингует все адреса
ПАРАЛЛЕЛЬНО (`spawn_mirror_probe`, `/api/policy`, таймаут 5с) и переключается на самый
быстрый ДОСТУПНЫЙ; недоступные не выбираются, не ответил никто — остаёмся на основном.
Окно входа при этом не ждёт пинг: стартуем на основном адресе, переключение приходит
фоном. Под живой сессией (`is_authenticated`/`is_loading`) авто-переключение не
срабатывает — игровая сессия и yggdrasil живут на том бэкенде, где начался вход.
Состояние хранится в `settings.json`: `api_url: null` = авто, `api_url: "<url>"` =
ручной выбор (его авто-пинг не перебивает).
...
- **Скриншоты переехали в нативный агент** (29.07.2026) — раньше кадр снимал процесс
  лаунчера: его убивали после старта игры (игра жила на heartbeat агента) → съёмка умирала,
  а на Windows GDI-`BitBlt` отдавал чёрный экран/застывший кадр рабочего стола, если игра
  в эксклюзивном фуллскрине. Теперь: **нативка** снимает (Windows — DXGI Desktop Duplication,
  фолбэк GDI; Linux — X11 через `dlopen` libX11, без жёсткой линковки), **Java-агент** только
  несёт кадр на бэкенд. Файловый протокол рядом с flag-файлом: `.key` (секрет подписи, пишет
  лаунчер, нативка читает и УДАЛЯЕТ в `Agent_OnLoad` до старта Java-кода) → `.shot.req` (id
  запроса от Java) → `.shot.raw` + `.shot.sig` (кадр и HMAC, `.sig` пишется последним = маркер
  готовности) / `.shot.err`. Кадр едет **PNG** (lossless): бэкенд разжимает, сверяет
  HMAC-SHA256 пикселей ключом сессии (`captureSecret` из `/handshake/init`, в JVM не попадает)
  и сам перекодирует в JPEG для хранения — подменить кадр модом внутри JVM нельзя.
  Старый канал (shot-token + готовый JPEG от лаунчера) оставлен для 0.4.3 и умрёт сам,
  когда все обновятся; каналы не пересекаются по флагу `capture=1` в flag-файле.
  ⚠️ Нужен релиз лаунчера **0.4.4** (без него ключ не кладётся → работает старый путь).
  ⚠️ Windows-путь (DXGI) собран кросс-компиляцией и НЕ проверен в рантайме — тут нет Windows.
  Подделать кадр нельзя, но канал можно ЗАГЛУШИТЬ (забрать запрос и промолчать), поэтому
  считается серия провалов: 3 подряд по игроку → детект `screenshot-failed` (severity 7,
  **confidence soft — не кикает**), сигнатура `screenshot-timeout` (клиент промолчал) или
  `screenshot-error` (клиент назвал причину). Удачный кадр обнуляет серию, пауза 30 мин —
  тоже. Soft намеренно: у части игроков захват не работает по среде (Wayland без XWayland,
...
**Аудит безопасности 30.07.2026 (20 фиксов, все с тестами).** Что важно знать дальше:
- **ADMIN_LOGINS сравнивается ТОЧНО (с регистром).** Была эскалация: матч ника был
  регистронезависимым, а уникальность `users.login` — регистрозависимой, т.е. регистрация
  «Likonchik» при админе «likonchik» давала роль admin любому желающему. Теперь значение в
  прод `.env` обязано совпадать с `users.login` побайтово. У текущего админа роль уже лежит
  в БД и держится веткой `IsPrivileged()`, так что доступ не потеряется. Заодно
  `repo.IsUsernameTaken` стал регистронезависимым — новые ники-двойники не создаются
  (существующие в БД остаются; проверить: `SELECT lower(login), count(*) FROM users GROUP BY 1 HAVING count(*)>1`).
  По той же причине ключ реестра отзывов доступа (`revoke.go`) больше не схлопывает регистр.
- **Протокол скриншотов изменился: `.shot.sig` теперь `<id>\n<подпись>\n<w> <h>\n`.**
  Первая строка — id запроса (лечит гонку «устаревший кадр → ложный `screenshot-failed`»).
  ⚠️ `libanticheat.so`/`anticheat.dll` и `anticheat-agent.jar` теперь **обязаны ехать на прод
  парой** — старый jar с новой нативкой прочитает id как подпись и завалит съёмку.
  ⚠️ `anticheat.dll` собран из ПРЕЖНИХ исходников: пересобрать на Windows-машине.
- `anticheat-agent/build/` в рабочей копии принадлежит root (собиралось под sudo/Docker) —
  `build.sh` падает на `rm -rf`. Артефакт в `backend/data/anticheat-agent.jar` обновлён
  сборкой мимо этого каталога; чтобы вернуть `build.sh`, нужен
  `sudo chown -R liko:liko anticheat-agent/build`.
- `EnableIPValidation: true` в Fiber-конфиге: без него `c.IP()` отдавал СЫРУЮ строку XFF —
  клиент дописывал в неё что угодно и получал новый ключ брутфорс-лимитера на каждую попытку.
- **keyring в лаунчере собран БЕЗ бэкендов** (`keyring = "3"` без features → mock in-memory).
  Это не дыра (JWT на диск не пишется вообще), но токен не переживает перезапуск: игрок
  логинится каждый старт. Нужна фича `sync-secret-service`/`windows-native`, если хочется
  «запомнить меня».
...
Текущие версии:
- **0.5.4** (после успешного обновления сборки отбрасываются результаты локальных
  проверок, начатых на старом manifest; карточка без перезапуска переходит в актуальное
  состояние «Готово к запуску» или показывает новое серверное обновление).
...
- **0.4.4** (нативная съёмка скриншотов + фиксы аудита безопасности 30.07.2026, см. ниже).
  ...
  файла (ELF/MZ). Подпись Ed25519 покрывает только байты и не говорит, ЧТО это за релиз,
  поэтому подписанный старый бинарник можно было выдать за новую версию (канал обновлений
  полу-доверенный: зеркало терминирует TLS само). Следствие: **все релизы, заливаемые для
  лаунчеров ≥0.4.4, обязаны собираться из кода с маркером** (т.е. ≥0.4.4). Пере-заливка
  старого бинарника как «последней версии» будет отклонена клиентом — это by design.
  Маркер держится в бинарнике тем, что реально уходит в заголовок `X-Launcher-Build`
  запроса `/api/launcher/update`; тест `current_binary_declares_its_version` это сторожит.
