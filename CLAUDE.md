...
- `launcher-slint/` — десктоп-лаунчер на Rust + Slint. Текущая версия: `0.4.2` (в `Cargo.toml`).
...
- **0.4.2** (аудит безопасности: подпись автообновления Ed25519, JWT только в keyring, no-shell open-url) —
  собрана только под Linux, НЕ залита. ⚠️ Подпись обновления вшивается ТОЛЬКО если при сборке задан
