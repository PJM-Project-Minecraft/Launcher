...
- `launcher-slint/` — десктоп-лаунчер на Rust + Slint. Текущая версия: `0.4.0` (в `Cargo.toml`).
...
Текущие версии:
- **0.4.0** (видимые ошибки синка на карточке + `sync-errors.log` + детект мгновенного краша игры с `launch.log`) — в работе, НЕ залит.
