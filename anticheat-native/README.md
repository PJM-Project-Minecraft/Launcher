...
- **скриншоты (`screenshot.c`)**: по запросу админа снимает экран. Windows — DXGI Desktop
  Duplication (фолбэк GDI), Linux — X11 через `dlopen` libX11.
...
### Скриншоты: почему здесь, а не в лаунчере

Раньше кадр снимал процесс лаунчера. Две дыры:

1. **Обход.** Лаунчер убивался после старта игры (игра продолжала жить на heartbeat
   Java-агента) — и съёмка умирала. Нативка живёт внутри игровой JVM: убить = закрыть игру.
2. **Чёрный экран.** GDI-`BitBlt` с DC экрана не видит кадр игры в эксклюзивном
   полноэкранном режиме → чёрный кадр или последняя composed-картинка рабочего стола
   («застывший» кадр). DXGI Desktop Duplication снимает вывод монитора и от режима не зависит.

Съёмка в нативке, доставка — Java-агентом (у нативки нет HTTPS, у Java нет доступа к
экрану поверх фуллскрина). Так как кадр проходит через JVM, где может жить чит-мод,
нативка **подписывает пиксели**:

```
<flag>.key      секрет (64 hex) — пишет ЛАУНЧЕР до старта JVM;
                нативка читает и УДАЛЯЕТ его в Agent_OnLoad, т.е. до первого Java-класса
<flag>.shot.req id запроса — пишет Java-агент, получив запрос с бэкенда
<flag>.shot.raw кадр RGB24 построчно (ширина ужимается до 1920)
<flag>.shot.sig "<hex HMAC>\n<w> <h>\n" — пишется ПОСЛЕДНИМ = маркер готовности
<flag>.shot.err причина неудачи (нет libX11, DXGI недоступен и т.п.)

подпись = HMAC-SHA256(секрет, "<id>\n<w>\n<h>\n" || сырые RGB)
```

Java-агент кодирует кадр в **PNG** (сжатие обязано быть без потерь) и грузит вместе с
подписью; бэкенд разжимает PNG, сверяет HMAC по тем же пикселям и сам перекодирует в JPEG
для хранения. Секрет выдаёт бэкенд на `/handshake/init` (`captureSecret`) — в JVM он не
попадает, поэтому подменить кадр мод не может.

Флаг-файл получил поле `capture=1` — им нативка сообщает Java-агенту, что ключ получен и
опрашивать бэкенд можно. При `capture=0` (старый лаунчер, ключа нет) канал остаётся за
лаунчером: два опросчика на одну очередь запросов передрались бы.
